...
void bus_clock(BUS_t* bus) {
    ...
    switch (device_type) {
        case DT_CPU:
            //log_msg(LP_INFO, "BUS %d: Attending to a CPU", bus->clock);
            switch (device_state) {
                case DS_IDLE:
                    //log_msg(LP_INFO, "BUS %d: The CPU is idle", bus->clock);
                    break;
                
                case DS_FETCH:
                    //log_msg(LP_INFO, "BUS %d: The CPU is fetching data", bus->clock);
                    if (device->processed == 1) {
                        //log_msg(LP_INFO, "BUS %d: The CPU has a fulfilled request pending", bus->clock);
                        break;
                    }
                    // Ok, so lets find the RAM device, to route the request to it
                    {
                        ...
                        //log_msg(LP_INFO, "BUS %d: Found RAM device to fetch data from. Making request", bus->clock);
                        ...
                        if (device_ram->device_state == DS_IDLE) {
                            ...
                            //log_msg(LP_INFO, "BUS %d: RAM device is Idle, need to wait", bus->clock);
                        }
                    }
                    break;
                
                case DS_STORE:
                    //log_msg(LP_INFO, "BUS %d: The CPU is storing data", bus->clock);
                    if (device->processed == 1) {
                        //log_msg(LP_INFO, "BUS %d: The CPU has a fulfilled request pending", bus->clock);
                        break;
                    }
                    // Ok, so lets find the RAM device, to route the request to it
                    {
                        ...
                        //log_msg(LP_INFO, "BUS %d: Found RAM device to store data to. Making request", bus->clock);
                        // lets just straight up overwrite it and see what happens
                        if (device_ram->device_state == DS_IDLE) {
                            device_ram->address = device->address;
                ...
                default:
                    //log_msg(LP_INFO, "BUS %d: The CPU is in an unknown state %d", bus->clock, device_state);
                    break;
            }
            break;
        ...
        case DT_RAM:
            //log_msg(LP_INFO, "BUS %d: Attending to a RAM", bus->clock);
            
            // figure out what the CPU wants
            switch (device_state) {
                case DS_IDLE:
                    //log_msg(LP_INFO, "BUS %d: The RAM is idle", bus->clock);
                    break;
                
                case DS_FETCH:
                    //log_msg(LP_INFO, "BUS %d: The RAM is retrieving data", bus->clock);
                    if (device->processed == 0) {
                        //log_msg(LP_INFO, "BUS %d: The RAM is not done with the request yet", bus->clock);
                        break;
                    }
                    {
                        ...
                        //log_msg(LP_INFO, "BUS %d: Found Target device to send data to", bus->clock);
                ...
                case DS_STORE:
                    //log_msg(LP_INFO, "BUS %d: The RAM is storing data", bus->clock);
                    if (device->processed == 0) {
                        //log_msg(LP_INFO, "BUS %d: The RAM is not done with the request yet", bus->clock);
                        break;
                    }
                    {
                        ...
                        //log_msg(LP_INFO, "BUS %d: Found Target device to validate", bus->clock);
                        device_target->processed = 1;
                        device->device_state = DS_IDLE;
                    }
                    break;
                ...
                default:
                    //log_msg(LP_INFO, "BUS %d: The RAM is in an unknown state %d", bus->clock, device_state);
                    break;
            }
            break;
        ...
        case DT_CLOCK:
            switch (device_state) {
                case DS_IDLE:
                    //log_msg(LP_INFO, "BUS %d: The CLOCK is idle", bus->clock);
                    break;
                
                case DS_INTERRUPT:
                    //log_msg(LP_INFO, "BUS %d: The CLOCK has sent an interrupt signal", bus->clock);
                    {
                        ...
                        device->device_state = DS_IDLE;
                        //log_msg(LP_INFO, "BUS %d: Target device (CPU) notified", bus->clock);
                    }
                    break;
                
                default:
                    //log_msg(LP_INFO, "BUS %d: The CLOCK is in an unknown state %d", bus->clock, device_state);
                    break;
                }
            break;
